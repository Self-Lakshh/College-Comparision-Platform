...
const CompareTable = ({ colleges }) => {
  ...
  return (
    <ScrollArea className="w-full whitespace-nowrap rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-sm overflow-hidden">
      <div className="min-w-max">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-zinc-200 dark:border-zinc-800">
              <th className="p-6 bg-zinc-50 dark:bg-zinc-900/80 text-zinc-500 dark:text-zinc-400 text-[10px] font-bold uppercase tracking-[0.2em] sticky left-0 z-20 min-w-[200px] backdrop-blur-md border-r border-zinc-200 dark:border-zinc-800">
                Specifications
              </th>
              {colleges.map((college) => (
                <th key={college._id} className="p-6 min-w-[320px] border-l border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950">
                  <div className="space-y-4">
                    <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 whitespace-normal leading-tight tracking-tight">
                      {college.name}
                    </h3>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.key} className="border-b border-zinc-200 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
                <td className="p-6 bg-zinc-50 dark:bg-zinc-900/80 text-zinc-500 dark:text-zinc-400 text-[11px] font-bold uppercase tracking-widest sticky left-0 z-10 border-r border-zinc-200 dark:border-zinc-800 backdrop-blur-md">
                  {row.label}
                </td>
                {colleges.map((college) => (
                  <td key={college._id} className="p-6 text-zinc-900 dark:text-zinc-300 text-sm border-l border-zinc-200 dark:border-zinc-800 font-medium">
                    {row.render(college)}
                  </td>
                ))}
              </tr>
